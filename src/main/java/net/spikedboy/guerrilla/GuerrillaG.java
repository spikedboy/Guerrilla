...
public class GuerrillaG implements Serializable {
    ...
    public void addSafeChest(Chest chest, Player sender) {
        ...
        if (Guerrilla.minPSC > np) {
            sender.sendMessage(Guerrilla.gCh + "You can't have a safe chest if your guerrilla has less than "
                    + Guerrilla.minPSC + " members");
            return;
        }
    ...
    public boolean sbonline() {
        for (String pname : Players) {
            Collection<? extends Player> onlinePlayers = Guerrilla.sinst.getOnlinePlayers();
        ...
        if (GuerrillaG.getPlayerGuerrilla(claimer) != null) {
            ...
            if ((Territories.contains(clist))) {
                ...
                claimer.sendMessage(ChatColor.DARK_RED + "[Guerrilla] " + ChatColor.GRAY + "The required price is "
                        + Guerrilla.chunkprice + " of " + (Material.getMaterial(Guerrilla.itemid).name()).toString()
                ...
                if (canClaim(chunk, claimer)) {
                    if (GuerrillaG.getGuerrillaChunk(chunk) != this && GuerrillaG.getGuerrillaChunk(chunk) != null) {
                        if (!claimer.getInventory().contains(Guerrilla.itemid, Guerrilla.chunkprice * Guerrilla.conqmulti)) {
                            claimer.sendMessage(ChatColor.DARK_RED + "[Guerrilla] " + ChatColor.GRAY + "The required price is "
                                    + (Guerrilla.chunkprice * Guerrilla.conqmulti) + " of "
                                    + (Material.getMaterial(Guerrilla.itemid).name()).toString() + " and you don't have it");
                        ...
                        GuerrillaG.getGuerrillaChunk(chunk).msggue(this.getName() + " is claiming part of your territory! Coords: "
    ...
    public boolean canClaim(Chunk chunk, Player claimer) {
        ...
        if ((owner != this) && (owner != null)) {
            ...
            if (((now.getTime() - owner.date.getTime()) >= Guerrilla.expTime) && ((adyChunks(chunk, owner) <= 2)
                    && (adyChunks(chunk, gclaimer) >= 1))) {
                return true;
            }
            if ((owner.quitPunishmentDate != null) && ((now.getTime() - owner.quitPunishmentDate.getTime()) < Guerrilla.delay)
    ...
    public void msggue(String msg) {
        for (String playername : Players) {
            ...
            if (Guerrilla.sinst.getPlayer(playername) != null) {
                Guerrilla.sinst.getPlayer(playername).sendMessage(ChatColor.DARK_RED + "[Guerrilla] " + ChatColor.GRAY + msg);
            }
        }
    }
        ...
        if (GuerrillaG.isLeader(sender)) {
            if (GuerrillaG.getPlayerGuerrilla(player) == GuerrillaG.getPlayerGuerrilla(sender)) {
                ...
                if (Guerrilla.sinst.getPlayer(player) != null) {
                    Guerrilla.sinst.getPlayer(player).sendMessage(ChatColor.DARK_RED + "[Guerrilla] " + ChatColor.GRAY
    ...
    public static void List(Player sender, int page) {
        ...
        sender.sendMessage(ChatColor.DARK_RED + "[Guerrilla] " + ChatColor.WHITE + "Listing Guerrillas:" + ChatColor.GRAY
                + " (Chunk objective: " + Guerrilla.objectiveChunks + ") " + ChatColor.WHITE + "PAGE " + page);
        for (int i = index; i < (GuerrillaList.size()); i++) {
            ...
            sender.sendMessage("- " + guerrilla.name + ChatColor.LIGHT_PURPLE + " Leader: " + ChatColor.WHITE
                    + guerrilla.getLeader() + ChatColor.LIGHT_PURPLE + " Chunks: " + ChatColor.WHITE + guerrilla.claimedchunks
                    + guerrilla.nonline());
        }
        return;
    }
            ...
            if (guerrilla == null) {
                ...
                if (guerrilla.quitPunishmentDate != null) {
                    sender.sendMessage(ChatColor.DARK_AQUA + " Punished: " + ChatColor.WHITE
                            + (((now.getTime() - guerrilla.quitPunishmentDate.getTime()) < Guerrilla.delay)));
                }
                sender.sendMessage(ChatColor.DARK_AQUA + " Members:");
                for (String playername : guerrilla.Players) {
    ...
    public static void removeMChests() {
        for (GuerrillaG guerrilla : GuerrillaList) {
            for (ArrayList<Integer> chestc : guerrilla.PaymentChests) {
                if (Guerrilla.sinst.getWorld(Guerrilla.gworldname).getBlockAt(chestc.get(0).intValue(),
