...
public class Guerrilla extends JavaPlugin {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args){
		...
		if (stateWon==true){
			playurd.sendMessage(gCh + "This round has ended, wait for an admin to start a new match! The winner was the " + gwinnerName + "'s guerrilla");
			return true;
		}
				...
				if ((args[0].equalsIgnoreCase("changeleader"))) {
					...
					if (args.length==2){
						if (args[1].equalsIgnoreCase("prices")) {
							...
							playurd.sendMessage(gCh + chunkprice + " " + Material.getMaterial(itemid).toString() + " for each unclaimed chunk claim, and " + chunkprice*conqmulti + " for every conquest");
							playurd.sendMessage(gCh + "Maintenance price: " + chunkmaintprice + " " + Material.getMaterial(itemidmaint).toString() + " every " + nchunkpay + " chunks, with a max of: " + maintmaxprice);
							if (guerrilla!=null) {
								int price = (int) (guerrilla.Territories.size()/Guerrilla.nchunkpay)*Guerrilla.chunkmaintprice;
								playurd.sendMessage(gCh + "You are currently paying: " + price + " per minecraft day. \nEach real day: " + (price*72)); 
							}
							return true;
						}
						int pnumber=0;
		...
		final Integer dclaimid = sinst.getScheduler().scheduleAsyncDelayedTask(ginst, new Runnable() {
		    public void run() {
		    	...
		    	if (claimer.getInventory().contains(Guerrilla.itemid, (Guerrilla.chunkprice*Guerrilla.conqmulti))) {
		    		GuerrillaG.removeInventoryItems(claimer.getInventory(), Material.getMaterial(Guerrilla.itemid), (Guerrilla.chunkprice*Guerrilla.conqmulti));
		    	} else {
